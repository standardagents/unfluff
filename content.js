(function(){
const he=/\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b/g,Ce=/https?:\/\/(www\.)?linkedin\.com\/in\/[^\s)]+/g,ye=/https?:\/\/(www\.)?(x\.com|twitter\.com)\/[A-Za-z0-9_]+/g,be=/(\+?\d{1,3}[-.\s]?)?\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}/g;function z(e){return e.replace(he,"[email]").replace(Ce,"[profile-url]").replace(ye,"[profile-url]").replace(be,"[phone]")}async function xe(e){const n=new TextEncoder().encode(e),o=await crypto.subtle.digest("SHA-256",n);return Array.from(new Uint8Array(o)).map(r=>r.toString(16).padStart(2,"0")).join("")}const $="unfluff-badge",N="unfluff-summary",T="unfluff-toggle",A="unfluff-original-toggle",re="unfluff-loading-placeholder",L="unfluff-pane",ae="unfluff-thumbnail-row",le="unfluff-thumbnail",b="data-unfluff-hidden";let f=null;function Se(e){f=e}const we={"Engagement Bait":"#cc1016","Humble Brag":"#b24020","Sales Pitch":"#8f5700","Motivational Fluff":"#9d2f72","Self-Promotion":"#c45200",Grift:"#a33a00","Rage Bait":"#d4200c","Clout Chasing":"#b85000","Virtue Signal":"#a82860","How-To":"#0a6b50",Opinion:"#4a6fa5","Case Study":"#2e6e9e",Technical:"#3b5998",Storytelling:"#7c5295",Investigative:"#1a7a5c",Analysis:"#2d6b9e","Life Update":"#0a66c2",Announcement:"#1b6f9a","Event Recap":"#5f3b8f","Job Posting":"#046336","Industry News":"#1f5a3f","Breaking News":"#c45200",Politics:"#4a6fa5",Shitpost:"#7c5295",Commentary:"#3b7a8f",PSA:"#046336"};function ve(e){let t=0;for(let o=0;o<e.length;o++)t=(t<<5)-t+e.charCodeAt(o),t|=0;return`hsl(${Math.abs(t)%360}, 55%, 38%)`}function Le(e){return we[e]??ve(e)}let R=!1;function ie(){if(R||!f)return;R=!0;const e=f.tokens,t=document.createElement("style");if(t.textContent=`
    ...
    .${A}:hover {
      ...
      background-color: ${H(e.textPrimary,.08)} !important;
      text-decoration: none !important;
    }
    .${T}:active,
    .${A}:active {
      background-color: ${H(e.textPrimary,.14)} !important;
    }
    .${A} {
      display: inline-block !important;
...
${o}`:"");I.set(i,e),W(e);const d={id:i,text:S};a.length>0?(d.imageUrls=a,console.log(`[Unfluff] Post ${i}: ${a.length} images ready`),k.push(d),B()):X(e)?Me(e,d):(k.push(d),B())}function X(e){if(!m)return!1;for(const t of m.imageSelectors){const n=t.replace(/\s+img$/,"");if(n!==t&&e.querySelector(n))return!0}for(const t of m.mediaItemSelectors)if(e.querySelector(t))return!0;return!1}function Me(e,t){const n=()=>{if(!m)return!1;const l=m.extractImageUrls(e);return l.length>0?(t.imageUrls=l,console.log(`[Unfluff] Post ${t.id}: ${l.length} images loaded (observer)`),!0):!1},o=()=>{const l=P.get(t.id);l&&(P.delete(t.id),l.observer.disconnect(),clearTimeout(l.timer),k.push(l.post),B())},a=new MutationObserver(()=>{n()&&o()});a.observe(e,{childList:!0,subtree:!0,attributes:!0,attributeFilter:["src"]});const r=setTimeout(()=>{console.log(`[Unfluff] Post ${t.id}: image watch timed out, sending without images`),o()},3e3);P.set(t.id,{post:t,observer:a,timer:r})}function B(){E||(E=!0,requestAnimationFrame(()=>{E=!1,Ve()}))}function Ve(){if(k.length===0)return;const e=k.splice(0),t=e.filter(o=>o.imageUrls&&o.imageUrls.length>0);console.log(`[Unfluff] Sending ${e.length} posts to service worker (${t.length} with images)`);const n={type:"ANALYZE_BATCH",posts:e};chrome.runtime.sendMessage(n,o=>{if(chrome.runtime.lastError){console.error("[Unfluff] Message error:",chrome.runtime.lastError.message);return}if(!o){console.error("[Unfluff] No response from service worker");return}if(o.type==="BATCH_RESULT"){console.log(`[Unfluff] Received ${o.results.length} results`);for(const a of o.results){ue.set(a.id,{category:a.category??"",tags:a.tags??[],translation:a.translation??"",attachmentCount:a.attachmentCount??0,attachmentSummary:a.attachmentSummary,cached:a.cached});const r=I.get(a.id);if(!r){console.warn(`[Unfluff] No DOM element for post ${a.id}`);continue}if(!r.isConnected){console.log(`[Unfluff] Element for ${a.id} removed from DOM, result cached for re-appearance`);continue}try{fe(r,a.category??"",a.tags??[],a.translation,a.attachmentCount??0,a.attachmentSummary,a.cached)}catch(l){console.error(`[Unfluff] Failed to render badge for ${a.id}:`,l)}}}else o.type==="BATCH_ERROR"?console.error("[Unfluff] Analysis error:",o.error):console.error("[Unfluff] Unexpected response type:",o)})}function Ne(){if(!m)return;const e=document.querySelectorAll(m.postSelector);for(const t of e)O(t)}function q(e){m=e,Se(e),w&&(w.disconnect(),w=null),Ne();const t=window!==window.top,n=document.querySelector(m.feedSelector);if(console.log("[Unfluff:observer] startObserver — feedSelector:",m.feedSelector,"| feed found:",!!n,"| isIframe:",t,"| href:",location.href),!n){console.log("[Unfluff:observer] Feed not found, retrying in 1s..."),setTimeout(()=>q(e),1e3);return}w=new MutationObserver(o=>{for(const a of o)for(const r of a.addedNodes){if(!(r instanceof Element))continue;r.matches(m.postSelector)&&O(r);const l=r.querySelectorAll(m.postSelector);for(const s of l)O(s)}}),w.observe(n,{childList:!0,subtree:!0}),console.log("[Unfluff] Observer started, monitoring feed for new posts")}function M(e){w&&(w.disconnect(),w=null),I.clear(),k.length=0,E=!1;for(const n of P.values())n.observer.disconnect(),clearTimeout(n.timer);P.clear(),qe();const t=document.querySelectorAll(e.postSelector);for(const n of t)e.isPromotedPost(n)&&n.style.display==="none"&&(n.style.display="");console.log("[Unfluff] Observer stopped — badges removed, content restored")}const Z="unfluff-cleanup";let v=null;function He(e){if(document.getElementById(Z))return;const t=document.createElement("style");t.id=Z,t.textContent=e.cleanupCss,document.head.appendChild(t)}function Re(){var e;(e=document.getElementById(Z))==null||e.remove()}function Oe(){const e=document.querySelectorAll('[data-unfluff-hidden-by-cleanup="true"]');for(const n of e)n.style.display="",n.removeAttribute("data-unfluff-hidden-by-cleanup");const t=document.querySelectorAll("[data-unfluff-checked]");for(const n of t)n.removeAttribute("data-unfluff-checked")}function U(e){v&&(v.disconnect(),v=null),He(e);function t(){e.runCleanup(document)}t();const n=document.querySelector(e.feedSelector);if(!n){setTimeout(()=>U(e),1e3);return}v=new MutationObserver(()=>{requestAnimationFrame(t)}),v.observe(n,{childList:!0,subtree:!0}),console.log("[Unfluff] Cleanup started — hiding sidebar, promoted, and recommended content")}function V(){v&&(v.disconnect(),v=null),Re(),Oe(),console.log("[Unfluff] Cleanup stopped — restored hidden content")}const Y="unfluff-toggle-styles";function de(e,t,n){Be(t);const o=chrome.runtime.getURL("fluff.webp"),a=chrome.runtime.getURL("nofluff.webp"),r=document.createElement("button");r.className="unfluff-toggle-btn",r.type="button",r.setAttribute("aria-label",e?"Unfluff is active — click to disable":"Unfluff is inactive — click to enable");const l=document.createElement("div");l.className=`unfluff-col ${e?"unfluff-col--dim":"unfluff-col--active"}`;const s=document.createElement("img");s.className="unfluff-sheep",s.src=o,s.alt="Fluff",s.width=24,s.height=24;const i=document.createElement("span");i.className=`unfluff-col-label ${e?"":"unfluff-col-label--active"}`,i.textContent="Fluff",l.appendChild(s),l.appendChild(i);const u=document.createElement("div");u.className=`unfluff-pill ${e?"unfluff-pill--on":"unfluff-pill--off"}`;const p=document.createElement("div");p.className=`unfluff-pill-knob ${e?"unfluff-pill-knob--on":"unfluff-pill-knob--off"}`,u.appendChild(p);const C=document.createElement("div");C.className=`unfluff-col ${e?"unfluff-col--active":"unfluff-col--dim"}`;const c=document.createElement("img");c.className="unfluff-sheep",c.src=a,c.alt="No Fluff",c.width=24,c.height=24;const S=document.createElement("span");S.className=`unfluff-col-label ${e?"unfluff-col-label--active":""}`,S.textContent="No Fluff",C.appendChild(c),C.appendChild(S),r.appendChild(l),r.appendChild(u),r.appendChild(C);let d=e;function h(g){l.className=`unfluff-col ${g?"unfluff-col--dim":"unfluff-col--active"}`,i.className=`unfluff-col-label ${g?"":"unfluff-col-label--active"}`,C.className=`unfluff-col ${g?"unfluff-col--active":"unfluff-col--dim"}`,S.className=`unfluff-col-label ${g?"unfluff-col-label--active":""}`,u.className=`unfluff-pill ${g?"unfluff-pill--on":"unfluff-pill--off"}`,p.className=`unfluff-pill-knob ${g?"unfluff-pill-knob--on":"unfluff-pill-knob--off"}`,r.setAttribute("aria-label",g?"Unfluff is active — click to disable":"Unfluff is inactive — click to enable")}return r.addEventListener("click",async()=>{d=!d,await chrome.storage.local.set({enabled:d}),h(d),d?(n.enable(),console.log("[Unfluff] Enabled via toggle")):(n.disable(),console.log("[Unfluff] Disabled via toggle"))}),chrome.storage.onChanged.addListener((g,y)=>{if(y!=="local"||!g.enabled)return;const x=g.enabled.newValue!==!1;x!==d&&(d=x,h(x),x?(n.enable(),console.log("[Unfluff] Enabled via external toggle (popup)")):(n.disable(),console.log("[Unfluff] Disabled via external toggle (popup)")))}),{element:r,updateState(g){d=g,h(g)}}}function Be(e){if(document.getElementById(Y))return;const t=document.createElement("style");t.id=Y,t.textContent=`
  ...
  `,document.head.appendChild(t)}const K="unfluff-nav-toggle",Q=[".feed-shared-update-v2__description .break-words",".feed-shared-text",".update-components-text",'span[dir="ltr"]'],Ze=[".update-components-actor__name .visually-hidden",".feed-shared-actor__name",".update-components-actor__title .visually-hidden"],Fe=["promoted","gesponsert","sponsorisé","patrocinado"],De=[".feed-shared-update-v2__update-content-wrapper",".update-components-mini-update-v2",".feed-shared-mini-update"],je=[".feed-shared-image__image",".feed-shared-carousel__image",".update-components-image img","img[data-delayed-url]"],ze=[".feed-shared-article__title",".update-components-article__title",".feed-shared-mini-update-v2__title",".artdeco-entity-lockup__title"],We=[".feed-shared-article__subtitle",".update-components-article__subtitle",".feed-shared-article__description"],Ge=`
...
`,J=/recommended for you|jobs recommended/i,Xe=/grow your business faster|try premium page|advertise on linkedin/i;function Ye(e,t){const n=e.querySelectorAll(t);for(const o of n)if(!o.dataset.unfluffChecked&&(o.dataset.unfluffChecked="true",me.isPromotedPost(o))){const a=o.closest("[data-id]");a?(a.style.display="none",a.dataset.unfluffHiddenByCleanup="true"):(o.style.display="none",o.dataset.unfluffHiddenByCleanup="true")}}function Ke(e,t){var a;const n=e.querySelectorAll(".update-components-header__text-view");for(const r of n){const l=((a=r.textContent)==null?void 0:a.trim())||"";if(!J.test(l))continue;const s=r.closest(".scaffold-finite-scroll__content");if(!s)continue;let i=r;for(;i&&i.parentElement!==s;)i=i.parentElement;i&&!i.dataset.unfluffChecked&&(i.dataset.unfluffChecked="true",i.style.display="none",i.dataset.unfluffHiddenByCleanup="true")}const o=e.querySelector(".scaffold-finite-scroll__content");if(o)for(const r of o.children){if(r.dataset.unfluffChecked||r.querySelector(t))continue;const l=r.textContent||"";J.test(l)&&(r.dataset.unfluffChecked="true",r.style.display="none",r.dataset.unfluffHiddenByCleanup="true")}}function Qe(e){const t=e.querySelector(".scaffold-layout__sidebar");if(t)for(const n of t.querySelectorAll("footer.artdeco-card__actions"))n.dataset.unfluffChecked||(n.dataset.unfluffChecked="true",Xe.test(n.textContent||"")&&(n.style.display="none",n.dataset.unfluffHiddenByCleanup="true"))}const me={name:"linkedin",hostnames:["linkedin.com"],isFeedPage(){var a;const e=location.pathname.startsWith("/feed");let t=null,n=!1;try{t=((a=window.frameElement)==null?void 0:a.getAttribute("data-testid"))??null,n=t==="interop-iframe"}catch{}const o=e||n;return console.log("[Unfluff:linkedin:isFeedPage]",{pathname:location.pathname,pathMatch:e,frameTestId:t,frameMatch:n,result:o}),o},feedSelector:"main",postSelector:"div.feed-shared-update-v2",extractPostText(e){var t;for(const n of Q){const o=e.querySelector(n);if((t=o==null?void 0:o.textContent)!=null&&t.trim())return o.textContent.trim()}return""},extractAuthorName(e){var t;for(const n of Ze){const o=e.querySelector(n);if((t=o==null?void 0:o.textContent)!=null&&t.trim())return o.textContent.trim()}return""},extractReshareText(e){var t;for(const n of De){const o=e.querySelector(n);if(!o)continue;const a=[];for(const r of Q)for(const l of o.querySelectorAll(r)){const s=(t=l.textContent)==null?void 0:t.trim();s&&a.push(s)}if(a.length>0)return a.join(`
`)}return""},extractImageUrls(e){const t=[];for(const n of je)for(const o of e.querySelectorAll(n)){const a=o.getAttribute("src")||o.getAttribute("data-delayed-url");a&&a.startsWith("http")&&!t.includes(a)&&t.push(a)}return t.slice(0,3)},extractEmbedContext(e){var n,o;const t=[];for(const a of ze){const r=e.querySelector(a);if((n=r==null?void 0:r.textContent)!=null&&n.trim()){t.push(r.textContent.trim());break}}for(const a of We){const r=e.querySelector(a);if((o=r==null?void 0:r.textContent)!=null&&o.trim()){t.push(r.textContent.trim());break}}return t.length===0?"":`[Embedded article: ${t.join(" — ")}]`},getPostId(e){const t=e.getAttribute("data-urn");if(t)return t;const n=e.closest("[data-urn]");if(n){const o=n.getAttribute("data-urn");if(o)return o}return null},isPromotedPost(e){var o;const t=e.querySelectorAll(".update-components-actor__sub-description, .update-components-actor__description, .feed-shared-actor__sub-description");for(const a of t){const r=((o=a.textContent)==null?void 0:o.toLowerCase().trim())||"";if(Fe.some(l=>r.includes(l)))return!0}return!!e.querySelector('a[href*="about/ads"]')},badgeAnchorSelector:".update-components-actor__sub-description",textSelectors:[".feed-shared-update-v2__description",".update-components-text"],mediaWrapperSelectors:[".feed-shared-update-v2__content",".update-components-content"],reshareSelectors:[".feed-shared-update-v2__update-content-wrapper"],mediaItemSelectors:[".feed-shared-image",".update-components-image",".feed-shared-carousel",".feed-shared-article",".update-components-article",".feed-shared-external-video",".update-components-linkedin-video",".update-components-video",".feed-shared-mini-update",".update-components-mini-update-v2",".feed-shared-document",".update-components-document",".feed-shared-poll",".update-components-poll",".feed-shared-celebration",".update-components-celebration"],commentSelectors:[".feed-shared-update-v2__comments-container",".comments-comments-list",".comments-comment-entity"],imageSelectors:[".feed-shared-image__image",".feed-shared-carousel__image",".update-components-image img","img[data-delayed-url]"],surplusCountSelectors:[".feed-shared-image-gallery__surplus-count",".update-components-image__surplus-count",".feed-shared-image__surplus-count"],injectNavToggle(e,t){var p,C;if(document.getElementById(K))return;const n=document.querySelector("nav.global-nav__nav ul.global-nav__primary-items");if(!n){setTimeout(()=>this.injectNavToggle(e,t),1e3);return}const o=n.querySelectorAll(":scope > li.global-nav__primary-item");if(o.length<2)return;const a=o[o.length-1],r=o[o.length-2],l=((p=a.textContent)==null?void 0:p.trim())||"",s=((C=r.textContent)==null?void 0:C.trim())||"";(/for business/i.test(s)||/try for/i.test(l)||a.querySelector(".premium-upsell-link")||r.querySelector(".global-nav__app-launcher-menu"))&&(a.remove(),r.remove());const i=document.createElement("li");i.className="global-nav__primary-item",i.id=K,i.style.cssText="display:flex;align-items:center;justify-content:center;border-left:1px solid rgba(0,0,0,0.08);margin-left:4px;padding-left:4px;";const u=de(e,"#0a66c2",t);i.appendChild(u.element),n.appendChild(i)},cleanupCss:Ge,runCleanup(e){Ye(e,this.postSelector),Ke(e,this.postSelector),Qe(e)},tokens:{textPrimary:"rgba(0,0,0,0.9)",textSecondary:"rgba(0,0,0,0.6)",textMuted:"rgba(0,0,0,0.4)",border:"rgba(0,0,0,0.08)",accent:"#0a66c2",skeletonBg:"rgba(0,0,0,0.06)",font:"-apple-system, system-ui, 'Segoe UI', Roboto, Oxygen, Ubuntu, sans-serif"},badgeCss:`
    .unfluff-pane { margin: 8px 16px; background: #f8f8f8 !important; }
    .unfluff-original-toggle { margin: 8px 16px; }
  `,seeMoreSelector:".feed-shared-inline-show-more-text, .feed-shared-inline-show-more-text__see-more-less-toggle"},ee="unfluff-nav-toggle";function te(e){const t=e.match(/(\d+)\s*,\s*(\d+)\s*,\s*(\d+)/);if(!t)return"lightsOut";const n=+t[1],o=+t[2],a=+t[3];return .299*n+.587*o+.114*a>128?"light":a>n+5?"dim":"lightsOut"}const Je=`
